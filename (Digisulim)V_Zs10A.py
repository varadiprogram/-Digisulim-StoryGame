...
def jatek():
    ...
    if kuldetesed == "F":
        ...
        ujra_kezdes = input("Megpróbálod még egyszer a játékot? (igen/nem): ")
        if ujra_kezdes == "igen":
            print()
        elif ujra_kezdes == "nem":
            koszonom_a_reszvetelt_meghaltal()

    while kuldetesed in ["D", "C", "B", "A", "S"]:
        print("\nNem tudsz a rangodtól 2-vel nagyobb küldetést vállalni.")
        time.sleep(1.5)
        kuldetesed = input(
    ...
    while segitesz != "nem" or segitesz != "igen":
        segitesz = input("Segítesz Neki? (igen/nem): ")
    ...
    while beakarsz_ujjitani != "igen" or beakarsz_ujjitani != "nem":
        beakarsz_ujjitani = input("Akarsz új tárgyakat venni?(igen/nem): ")
    ...
    if beakarsz_ujjitani == "igen":
        ...
        while vasarolsz_kardot != "igen" or vasarolsz_kardot != "nem":
            vasarolsz_kardot = input("Vásárolsz Kardot?(igen/nem): ")

        if vasarolsz_kardot == "igen":
            melyiket_akarod = int(input(
                "írd be azt hogy mennyi aranytállérba került: "))

            while melyiket_akarod != 999 or melyiket_akarod != 2499 or melyiket_akarod != 4999:
                melyiket_akarod = int(input(
                    "írd be azt hogy mennyi aranytállérba került: "))
            ...
            if melyiket_akarod == 999 and aranytaller > 999:
                ...
                erod = erod + 250
                print(
                    "Megvetted a kardot és az erőd nagyobb lett 200el, jelenlegi erő szinted: ", erod)

            elif melyiket_akarod == 2499 and aranytaller > 2499:
                aranytaller = aranytaller - 2499
                erod = erod + 850
                print(
                    "Megvetted a kardot és az erőd nagyobb lett 850el, jelenlegi erő szinted: ", erod)
    ...
    masodik_demon_ereje = random.randint(3000, 5200)

    time.sleep(2)
    print("\nTalálkozol az első démonnal akinek az ereje:", masodik_demon_ereje)
    ...
    harmadik_demon_ereje = random.randint(5200, 9300)

    time.sleep(2)
    print("\nTalálkozol az első démonnal akinek az ereje:", harmadik_demon_ereje)
    ...
    negyedik_demon_ereje = random.randint(9300, 16500)

    time.sleep(2)
    print("\nTalálkozol az első démonnal akinek az ereje:", negyedik_demon_ereje)
    ...
    if erod < demon_kiraly_ereje and erod < 30000:
        ...
        dontes = random.choice("Jobb", "bal")
        honnan_tamadsz = input("Honnan Támadsz?(jobb,bal): ")
        while honnan_tamadsz != "jobb" or honnan_tamadsz == "bal":
            honnan_tamadsz = input("Honnan Támadsz?(jobb,bal): ")
